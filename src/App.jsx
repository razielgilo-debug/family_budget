...
export default function App(){
  ...
  const [eomDone,setEomDone]=useState(null); // {action,amount,goal?} summary of what was done
  ...
  const VK  = offPK(CK, hOff);            // hOff>0 = past, hOff<0 = future
  ...
  async function eomToNext(){
    ...
    // Store action in prev month record
    const action=eomMode==="positive"?"לחודש הבא":"כוסה חריגה";
    const nhf={...nh,data:{...nh.data,[accId]:{...nh.data[accId],months:{...nh.data[accId].months,[PK]:{...(nh.data[accId].months?.[PK]||{}),eomAction:action,eomAmt:amt}}}}};
    await doSave(nhf);setShowEOM(false);
    setEomDone({action,amount:amt,key:PK});
    toast2(eomMode==="positive"?`₪${amt.toLocaleString()} הועברו לחודש הנוכחי 🎉`:`מינוס ₪${amt.toLocaleString()} הועבר לחודש הנוכחי`);
  }
  async function eomToSav(){
    const amt=Math.abs(prevRem);if(!eomSav){toast2("בחר יעד","err");return;}
    const savGoal=savings.find(g=>g.id===eomSav);
    ...
    const nhf={...nh,data:{...nh.data,[accId]:{...nh.data[accId],months:{...nh.data[accId].months,[PK]:{...(nh.data[accId].months?.[PK]||{}),eomAction:"לחיסכון",eomAmt:amt,eomGoal:savGoal?.name||""}}}}};
    await doSave(nhf);setShowEOM(false);
    setEomDone({action:"לחיסכון",amount:amt,goal:savGoal?.name,key:PK});
    toast2(`₪${amt.toLocaleString()} הועברו לחיסכון 🎯`);
  }
...
<table><thead><tr><th>קטגוריה</th><th>סה"כ</th><th>%</th></tr></thead><tbody>${catRows}</tbody></table>
  ...
  function getAnalysis(){
    ...
    // Exclude savings transfers from main analysis
    let ex=allExp.filter(e=>keys.includes(e.mKey)&&!e.transfer);
    const savTransferTotal=allExp.filter(e=>keys.includes(e.mKey)&&e.transfer).reduce((s,e)=>s+e.amount,0);
    ...
    // Overdraft per month
    const byMBud={};const overdraftM={};
    keys.forEach(k=>{byMBud[k]=getTotalBudget(accData,k);overdraftM[k]=Math.max(0,(byM[k]||0)-byMBud[k]);});
    return {keys,total,byCat,byM,byMBud,overdraftM,savTransferTotal};
  }
  ...
  if(!prof.onboarded) return(
    <div style={{fontFamily:"'Heebo',sans-serif",direction:"rtl",background:"#0F0F1A",minHeight:"100dvh",width:"100%",color:"#fff",display:"flex",flexDirection:"column",padding:"28px 24px",fontSize:15}}>
      ...
      `}</style>
      {/* Step dots — 2 steps if joining, 3 if creating */}
      <div style={{display:"flex",justifyContent:"center",gap:8,paddingTop:4,marginBottom:8}}>
        {[0,1,oUrl.trim()?null:2].filter(x=>x!==null).map((_, ii)=>(
          <div key={ii} style={{height:5,borderRadius:10,transition:"all .3s",
            width:ii===oStep?32:16,
            background:ii===oStep?"#6C63FF":ii<oStep?"rgba(108,99,255,0.5)":"rgba(255,255,255,0.15)"}}/>
        ))}
      </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center"}}>
        ...
        {/* ── Step 1: Sharing key (optional) + continue ── */}
        {oStep===1&&<>
          <div style={{textAlign:"center",marginBottom:22}}>
            <div style={{fontSize:48,marginBottom:10}}>🔑</div>
            <div style={{fontSize:22,fontWeight:800,marginBottom:6}}>מפתח שיתוף</div>
            <div style={{fontSize:13,color:"#888",lineHeight:1.8}}>
              קיבלת מפתח מבן/בת הזוג? הכנס אותו כאן.<br/>
              <b style={{color:"#4ECDC4"}}>תקבל את כל ההגדרות אוטומטית</b> — ללא בחירת יום איפוס.
            </div>
          </div>

          {/* URL input */}
          <div style={{marginBottom:14}}>
            <div style={{fontSize:12,color:"#888",marginBottom:6}}>Firebase URL (מפתח שיתוף)</div>
          ...
          {/* Join button — shown when URL is entered */}
          {oUrl.trim()&&<>
            <button className="ob-btn" onClick={finishOnboard}
              style={{background:"linear-gradient(135deg,#4ECDC4,#82E0AA)",color:"#111",marginBottom:10}}>
              {syncing?"מתחבר...":"🔗 הצטרף — קבל הגדרות אוטומטית"}
            </button>
            <div style={{fontSize:11,color:"#4ECDC4",textAlign:"center",marginBottom:14}}>
              ✓ לא תצטרך לבחור יום איפוס — ייקלט מהחשבון הקיים
            </div>
          </>}
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            ...
            <div style={{fontSize:12,color:"#555"}}>{oUrl.trim()?"או צור חדש":"אין לי מפתח"}</div>
            <div style={{flex:1,height:1,background:"rgba(255,255,255,0.1)"}}/>
          </div>

          {/* Create new — leads to reset day selection */}
          <button className="ob-btn" onClick={()=>{setOU("");setOS(2);}}
            style={{background:"linear-gradient(135deg,#6C63FF,#4ECDC4)"}}>
            {oUrl.trim()?"צור משק בית חדש במקום ›":"המשך לבחירת יום איפוס ›"}
          </button>

          {/* Firebase setup hint */}
          <div style={{marginTop:16,padding:"10px 14px",background:"rgba(108,99,255,0.08)",border:"1px solid rgba(108,99,255,0.2)",borderRadius:12,fontSize:11,color:"#777",lineHeight:1.8}}>
            <b style={{color:"#999"}}>איך מקבלים מפתח שיתוף?</b><br/>
            1. <a href="https://console.firebase.google.com" target="_blank" rel="noreferrer" style={{color:"#4ECDC4"}}>console.firebase.google.com</a> → פרויקט חדש<br/>
            2. Build → Realtime Database → Create → <b style={{color:"#FFE66D"}}>TEST MODE</b><br/>
            3. העתק את ה-URL ושלח לבן/בת הזוג
          </div>
        </>}
    ...
    <div style={{fontFamily:"'Heebo',sans-serif",direction:"rtl",background:bg,minHeight:"100dvh",width:"100%",color:tc,position:"relative",zoom}}>
      ...
      {editExp&&<BS onClose={()=>setEditExp(null)}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onMouseDown={e=>e.preventDefault()} onClick={()=>setEditExp(null)} style={{flex:1,padding:11,borderRadius:12,border:`1px solid ${cbr}`,background:"transparent",color:tc,cursor:"pointer",fontWeight:700,fontFamily:"Heebo"}}>ביטול</button>
          <button onMouseDown={e=>e.preventDefault()} onClick={async()=>{await delExp(editExp.id);setEditExp(null);toast2("נמחק 🗑");}}
          ...
          <button onMouseDown={e=>e.preventDefault()} onClick={saveEditExp} className="btn" style={{flex:2,padding:11}}>שמור</button>
        </div>
      </BS>}
      ...
      {depG&&<BS onClose={()=>{setDepG(null);setDepA("");}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onMouseDown={e=>e.preventDefault()} onClick={()=>{setDepG(null);setDepA("");}} style={{flex:1,padding:12,borderRadius:12,border:`1px solid ${cbr}`,background:"transparent",color:tc,cursor:"pointer",fontWeight:700,fontFamily:"Heebo"}}>ביטול</button>
          <button onMouseDown={e=>e.preventDefault()} onClick={deposit} className="btn" style={{flex:1,padding:12}}>הפקד</button>
        </div>
      </BS>}
      {wdG&&<BS onClose={()=>{setWdG(null);setWdA("");}}>
        {(()=>{const g=savings.find(x=>x.id===wdG);return(<>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onMouseDown={e=>e.preventDefault()} onClick={()=>{setWdG(null);setWdA("");}} style={{flex:1,padding:12,borderRadius:12,border:`1px solid ${cbr}`,background:"transparent",color:tc,cursor:"pointer",fontWeight:700,fontFamily:"Heebo"}}>ביטול</button>
            <button onMouseDown={e=>e.preventDefault()} onClick={withdraw} style={{flex:1,padding:12,borderRadius:12,border:"none",background:"linear-gradient(135deg,#FF8B94,#FF6B6B)",color:"#fff",cursor:"pointer",fontWeight:700,fontFamily:"Heebo"}}>משוך</button>
          </div>
        </>);})()}
      </BS>}
      {showXfer&&<BS onClose={()=>{setShowXfer(false);setXG("");setXA("");}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onMouseDown={e=>e.preventDefault()} onClick={()=>{setShowXfer(false);setXG("");setXA("");}} style={{flex:1,padding:12,borderRadius:12,border:`1px solid ${cbr}`,background:"transparent",color:tc,cursor:"pointer",fontWeight:700,fontFamily:"Heebo"}}>ביטול</button>
          {savings.length>0&&<button onMouseDown={e=>e.preventDefault()} onClick={doXfer} className="btn" style={{flex:1,padding:12}}>העבר</button>}
        </div>
      </BS>}
      ...
      <div style={{padding:"12px 18px 115px"}}>
        ...
        {view==="home"&&<>
          ...
          <div className="card" style={{padding:20,marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              ...
              {isPastPeriod&&Math.abs(rem)>5&&<button className="btn" onClick={()=>{const pm=accData.months?.[VK];setEomMode(rem>0?"positive":"negative");if(!pm?.eomAction)setShowEOM(true);else toast2(rem>0?"יתרה: ₪"+rem.toLocaleString()+" הועברה "+pm.eomAction:"חריגה: ₪"+Math.abs(rem).toLocaleString()+" "+pm.eomAction);}} style={{flex:1,padding:"9px 0",fontSize:12,minWidth:80,background:"rgba(255,255,255,0.1)"}}>📋 סיכום תקופה</button>}
            </div>
          </div>
              ...
              <div key={e.id} className="erow" onClick={()=>setEditExp({...e})}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                    {e.biz&&<div style={{fontSize:13,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.biz}</div>}
                    {!e.biz&&<div style={{fontSize:13,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.cat.split(" ").slice(1).join(" ")}</div>}
                    ...
                    {e.recurringId&&<span style={{fontSize:9,background:"rgba(130,224,170,0.2)",border:"1px solid #82E0AA",borderRadius:10,padding:"1px 5px",color:"#82E0AA",flexShrink:0}}>🔄</span>}
                  ...
                  <div style={{fontSize:11,color:sub,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                    {e.biz?e.cat.split(" ").slice(1).join(" "):""}
                    {e.biz&&e.note?" · ":""}
                    {e.note||""}
                    {(e.biz||e.note)?" · ":""}{new Date(e.date).toLocaleDateString("he-IL")} · {e.by}
                  </div>
                </div>
          ...
          {/* EOM done summary */}
          {eomDone&&hOff===0&&<div style={{background:"rgba(78,205,196,0.1)",border:"1px solid rgba(78,205,196,0.3)",borderRadius:14,padding:"12px 16px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:13,fontWeight:700,color:"#4ECDC4"}}>✅ סיכום חודש קודם</div>
              <div style={{fontSize:12,color:sub}}>₪{eomDone.amount.toLocaleString()} — הועבר {eomDone.action}{eomDone.goal?` (${eomDone.goal})`:""}</div>
            </div>
            <button onClick={()=>setEomDone(null)} style={{background:"none",border:"none",color:sub,cursor:"pointer",fontSize:16,padding:"0 4px"}}>×</button>
          </div>}
          {/* EOM action stored in prev month data */}
          {hOff===1&&(()=>{const pm=accData.months?.[VK]||{};return pm.eomAction?<div style={{background:"rgba(78,205,196,0.08)",border:"1px solid rgba(78,205,196,0.25)",borderRadius:12,padding:"10px 14px",marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:700,color:"#4ECDC4",marginBottom:2}}>📋 מה נעשה עם יתרת התקופה</div>
            <div style={{fontSize:13,color:tc}}>₪{(pm.eomAmt||0).toLocaleString()} — {pm.eomAction}{pm.eomGoal?` (${pm.eomGoal})`:""}</div>
          </div>:null;})()}
              ...
              <div style={{display:"flex",alignItems:"center",gap:9}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:2}}>
                    ...
                    {e.iId&&<span style={{fontSize:9,background:"rgba(255,227,100,0.2)",border:"1px solid #FFE66D",borderRadius:10,padding:"1px 5px",color:"#FFE66D"}}>💳 תשלום {e.iIdx}/{e.iTotal} | ₪{(e.iSum||0).toLocaleString()} סה"כ | ₪{e.amount.toLocaleString()}/חודש</span>}
        ...
        {/* ════ ANALYSIS ══════════════════════════════════════════════════ */}
        {view==="analysis"&&(()=>{
          const {keys,total,byCat,byM,byMBud,overdraftM,savTransferTotal}=getAnalysis();
          ...
          return <div>
            ...
            <div className="card" style={{padding:16,marginBottom:savTransferTotal>0&&anCat==="הכל"?8:14,textAlign:"center"}}>
            ...
            {savTransferTotal>0&&anCat==="הכל"&&<div style={{marginBottom:14,padding:"8px 14px",background:"rgba(78,205,196,0.1)",border:"1px solid rgba(78,205,196,0.3)",borderRadius:12,fontSize:12,color:"#4ECDC4",display:"flex",justifyContent:"space-between"}}>
              <span>🎯 כסף שהועבר לחיסכון (לא כלול בניתוח)</span>
              <span style={{fontWeight:700}}>₪{savTransferTotal.toLocaleString()}</span>
            </div>}
            <div className="card" style={{padding:16,marginBottom:14}}>
              <div style={{fontSize:12,fontWeight:700,color:sub,marginBottom:10}}>לפי תקופה — ביחס לתקציב</div>
              {keys.map((k,i)=>{
                const sp=byM[k]||0,bud=byMBud[k]||0,od=overdraftM[k]||0;
                const pct2=bud>0?Math.min(130,Math.round((sp/bud)*100)):0;
                const isOver=od>0;
                const barColor=isOver?"#FF6B6B":pct2>85?"#FFE66D":CLR[i%CLR.length];
                const barW=bud>0?Math.min(100,(sp/bud)*100):sp>0?60:0;
                return <div key={k} style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,marginBottom:4}}>
                    <span style={{color:sub}}>{getPL(k,rd)}</span>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      {isOver&&<span style={{fontSize:10,background:"rgba(255,107,107,0.2)",border:"1px solid #FF6B6B",borderRadius:10,padding:"1px 6px",color:"#FF6B6B",fontWeight:700}}>חריגה ₪{od.toLocaleString()}</span>}
                      <span style={{fontWeight:700,color:isOver?"#FF6B6B":tc}}>₪{sp.toLocaleString()}{bud>0&&!isOver?<span style={{fontWeight:400,color:sub}}> / ₪{bud.toLocaleString()}</span>:""}</span>
                    </div>
                  </div>
                  <div style={{background:dark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.08)",borderRadius:8,height:10,position:"relative",overflow:"hidden"}}>
                    <div style={{width:`${barW}%`,height:10,borderRadius:8,background:isOver?"linear-gradient(90deg,#FF8B94,#FF6B6B)":pct2>85?`linear-gradient(90deg,#FFE66D,${CLR[i%CLR.length]})`:`linear-gradient(90deg,${CLR[i%CLR.length]},${acc})`,transition:"width .5s"}}/>
                    {bud>0&&<div style={{position:"absolute",top:0,left:"calc(100% - 2px)",width:2,height:10,background:"rgba(255,255,255,0.5)"}}/>}
                  </div>
                </div>;
              })}
            </div>
